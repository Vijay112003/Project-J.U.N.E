...
class ManualBloc extends Bloc<ManualEvent, ManualState> {
  ...
  void _onSendSyncMessage(SyncButtonPressed event, Emitter<ManualState> emit) {
    WebSocketHelper.sendMessage('SENDER', 'STATUS');
  }

  void _onSetBrightness(SetBrightness event, Emitter<ManualState> emit) {
    WebSocketHelper.sendMessage('SENDER', '{"type": "manual", "module": "brightness", "action": "set", "value": "${event.brightness}"}');
  }

  void _onSetVolume(SetVolume event, Emitter<ManualState> emit) {
    WebSocketHelper.sendMessage('SENDER', '{"type": "manual", "module": "volume", "action": "set", "value": "${event.volume}"}');
  }

  void _onTogglePower(TogglePower event, Emitter<ManualState> emit) {
    WebSocketHelper.sendMessage('SENDER', '{"type": "manual", "module": "power", "action": "toggle"}');
  }

  void _onToggleLock(ToggleLock event, Emitter<ManualState> emit) {
    WebSocketHelper.sendMessage('SENDER', '{"type": "manual", "module": "lock", "action": "toggle"}');
  }

  void _onToggleWifi(ToggleWifi event, Emitter<ManualState> emit) {
    WebSocketHelper.sendMessage('SENDER', '{"type": "manual", "module": "wifi", "action": "toggle"}');
  }

  void _onToggleBluetooth(ToggleBluetooth event, Emitter<ManualState> emit) {
    WebSocketHelper.sendMessage('SENDER', '{"type": "manual", "module": "bluetooth", "action": "toggle"}');
  }
}